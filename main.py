...
CONFIDENCE_THRESHOLD   = 60.0  # below this → low_confidence_readings
...
app = FastAPI(title="PaddleOCR Incremental Trainer", version="4.0.0")
...
def preprocess_and_save(img_path: Path) -> dict:
    """
    Exact OpenCV pipeline: grayscale -> 4x upscale -> bilateral -> CLAHE -> Otsu.
...
# Bypasses PaddleOCR 3.x high-level API entirely.
# Uses paddle.inference directly with the exported model.
# Works with en_PP-OCRv3_mobile_rec exported inference model.
# ─────────────────────────────────────────────────────────────────────────────

def _load_char_list(dict_path: Path) -> list:
    """Load character dictionary. Index 0 = blank (CTC)."""
    chars = ["blank"]
    if dict_path.exists():
        ...
        # fallback: digits + common symbols
...
def _build_predictor(infer_dir: Path):
    """
    Build Paddle predictor from exported model.
    Supports:
      - inference.pdmodel (old)
      - inference.json (new)
    """

    import paddle.inference as paddle_infer

    pdmodel_file = infer_dir / "inference.pdmodel"
    json_file = infer_dir / "inference.json"
    params_file = infer_dir / "inference.pdiparams"

    # choose model file
    if pdmodel_file.exists():
        model_file = str(pdmodel_file)

    elif json_file.exists():
        model_file = str(json_file)

    else:
        raise FileNotFoundError(
            f"No inference model found in {infer_dir}"
        )

    if not params_file.exists():
        raise FileNotFoundError(
            f"inference.pdiparams not found in {infer_dir}"
        )

    print(f"[OCR] Loading model from: {model_file}")
...
def _preprocess_for_rec(img_bgr: np.ndarray,
    ...
    """
    Resize image to [H, W] keeping aspect ratio, pad with black.
    Normalise to [0,1], subtract 0.5, divide by 0.5.
...
def _ctc_decode(preds: np.ndarray, char_list: list) -> tuple:
    ...
    text       = "".join(chars)
    confidence = round(float(np.mean(confs)) * 100, 2) if confs else 0.0
    return text, confidence


def _run_rec_on_image(img_bgr: np.ndarray,
                       predictor, char_list: list) -> tuple:
    """Run recognition predictor on one BGR image. Returns (text, confidence%)."""
    ...
    preds         = output_handle.copy_to_cpu()   # shape [1, T, C]

    return _ctc_decode(preds[0], char_list)


def find_latest_inference_dir() -> Optional[Path]:
    """Find the inference/ subdir of the highest trained version."""
    version_dirs = sorted(
    ...
    for vdir in reversed(version_dirs):
        ...
        if (
            (infer / "inference.pdiparams").exists()
            and (
                (infer / "inference.pdmodel").exists()
                or (infer / "inference.json").exists()
            )
        ):
            return infer
    return None
...
def get_ocr_predictor():
    """Return cached predictor + char_list, reloading if new version available."""
    ...
    print(f"[OCR] Loading predictor from: {infer_dir}")
    predictor = _build_predictor(infer_dir)
    char_list  = _load_char_list(DICT_FILE)

    _ocr_cache["version"]   = version
    _ocr_cache["predictor"] = predictor
    _ocr_cache["char_list"] = char_list
...
def _detect_regions(image: np.ndarray) -> list:
    """
    Your exact detect_regions() logic.
    Returns list of {"bbox": {x,y,width,height}, "crop": np.ndarray}
    Falls back to full image if no contours pass filters.
    """
    if len(image.shape) == 3:
        gray_det = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    else:
        gray_det = image.copy()
    ...
    for cnt in contours:
        ...
        area = w * h
        if area < 1000 or area > image_area * 0.9:
            continue
        if w < 80 or h < 40:
            continue
        pad = 20
    ...
    regions.sort(key=lambda r: r["bbox"]["width"] * r["bbox"]["height"],
                 reverse=True)
...
def save_state(state: dict):
    ...
    if not sid:
        return
    get_session_file(sid).write_text(
...
def find_previous_checkpoint() -> Optional[Path]:
    """Returns path WITHOUT .pdparams extension for PaddleOCR pretrained_model arg."""
    version_dirs = sorted(
        [p for p in MODELS_DIR.glob("v*/") if p.is_dir()],
...
async def run_training(state: dict):
    ...
    try:
        ...
        # Incremental: find previous checkpoint
        ...
        # Find saved .pdparams (PaddleOCR saves as latest/iter_epoch_N)
        ...
        if not best.exists():
            ...
            print(f"[Train] Saved best_accuracy from {saved[-1].name}")
        ...
        # Export inference model (needed by our custom predictor)
        ...
        if exp.returncode != 0:
            ...
            print(f"[Train] Inference model exported to: {infer_dir}")
            # Reload OCR cache with new model
            _ocr_cache["version"] = None

        # Keep only 2 latest version dirs
        all_vdirs = sorted(
            [p for p in MODELS_DIR.glob("v*/") if p.is_dir()],
            key=lambda p: int(p.name.replace("v","")) if p.name.replace("v","").isdigit() else 0
        )
...
async def prepare_and_train(state: dict):
    ...
    for img_name in sampled:
        # Preprocess old image if not done yet
        ...
        if not gray_path.exists() or not otsu_path.exists():
            ...
            if "error" in res:
                print(f"[Preprocess] Failed: {img_name} — {res['error']}")
            else:
                print(f"[Preprocess] Done: {img_name}")
...
# ─────────────────────────────────────────────────────────────────────────────
# STARTUP — backfill unprocessed images
# ─────────────────────────────────────────────────────────────────────────────
...
async def upload_image(
    ...
    """
    Upload one image. Gemini labels it, OpenCV preprocesses it.
...
async def extract(
    ...
    """
    OCR extraction using YOUR trained model (paddle.inference predictor).
    Bypasses PaddleOCR 3.x high-level API — no model name mismatch errors.
    Returns detailed response with readings, confidence, bbox, low-confidence warnings.
    """
    t_start = time.time()
    ...
    try:
        ...
        # Detect regions using your exact detect_regions() logic
        ...
        for region in regions:
            ...
            if confidence >= CONFIDENCE_THRESHOLD:
                ...
                low_conf_readings.append({
                    ...
                    "confidence": confidence,
                    "warning":    (
                        f"Confidence {confidence:.1f}% below "
